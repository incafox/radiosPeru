...
class MenuFavoritos extends StatefulWidget{
  //MenuRegion();
  List<RadioData> listaRadios = new List();
  @override
  _MenuFavoritos createState() => _MenuFavoritos();
}

class _MenuFavoritos extends State<MenuFavoritos>{
  ...
  void getFavoriteRadios()async{
    ...
    this.radiosFavoritas.forEach((radio) {print("[favoritos]" + radio.name);});
  }
        ...
        builder:  (BuildContext context, AsyncSnapshot<List<CartaRadio>> lista) {
          ...
          if (lista.connectionState == ConnectionState.done) {
            return GridView.count(crossAxisCount: 3,
              ...
              physics: BouncingScrollPhysics(),); // unreachable
          }
          else
            return Text("te vas a la mierda");
        },
      );
